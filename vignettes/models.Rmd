...
In one,
states are divided by hidden rates,
and in the other,
the rates might be the same on the tree,
but some states are divided into hidden states.
...
```{r}
new_rates_model(model = "IND", rates = c("a", "b"))
```

As in a simple model,
it is possible to create a multiple-rates model
for a single trait:

```{r}
new_rates_model(rates = c("a", "b", "c"), traits = 1)
```
...
```{r}
m <- new_rates_model(rates = c("a", "b", "c"), traits = 1)
m
...
```{r}
hrm <- new_rates_model(model = "IND", rates = c("a", "b"))
hrm
model_to_ard(hrm)
```
