...
The main purpose of models defined in `sinba` package
...
In two trait models,
the first trait is identified as *x*,
and the second trait is identified as *y*.

By default,
the function `new_model()`
creates a Pagel's independent model (1994)
for two traits:
...
and both are supported in `sinba` package.
In the first form,
...
If you want to set a specific parameter for a given cell,
you can use the function `model_set()`
to indicate the cell to be modified
and the parameter ID of the cell.

```{r}
m <- new_model("IND")
m
# set all transformation from 00 to have the same parameter
model_set(m, c(1, 3), 1)
# set the state 11 as an absorbing state
# (i.e., set to zero)
m <- model_set(m, c(4, 2), 0)
m <- model_set(m, c(4, 3), 0)
m
```
